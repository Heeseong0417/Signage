...
const [SelectItems, setSelectItems] = useState("홈")
  ...
  useEffect(() => {
    handleLogin(); // 컴포넌트가 마운트될 때 로그인 처리
 ...
 <div onClick={()=>router.push("/")} className=" cursor-pointer mx-2 flex flex-col lg:flex-row  items-center ">
          ...
          <div className=" justify-end lg:justify-between flex lg:text-[1rem] space-x-3 lg:space-x-6 text-xs md:text-base text-dimgray-200 min-w-[10rem] lg:ml-20">
            <div className=" lg:flex items-center space-x-1 lg:space-x-2 cursor-pointer">
              ...
              />
              <span onClick={()=>user.userId?clickMenu("/Login/Myprofile"):clickMenu("/Login/Siteup")} className="text-xs lg:text-sm font-extrabold text-nowrap">{user.userId?"내 정보":"회원가입"}</span>
            </div>
            <div className=" lg:flex items-center space-x-1 lg:space-x-2 cursor-pointer">
              <Image
                src="/rockon.svg"
                ...
                className="h-4 w-4 lg:h-5 lg:w-5"
              />
              <span onClick={()=>user.userId?handleLogout():clickMenu("/Login")} className="text-xs lg:text-sm font-extrabold text-nowrap">{user.userId?"로그아웃":"로그인"}</span>
            </div>
