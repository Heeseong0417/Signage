...
const Header= ({children}:any)=>{
  const [user, setuser] = useState<any>({role:null})
  const menu = [
    {title:"대시보드",link:"/Home/ADuser/Dashboard",sub:[]},
    {title:"옥외광고",link:"/Home/data",sub:[{title:"광고소개",link:"/Home/ADuser/Introduction"},{title:"광고단가 및 약관안내",link:"/Home/ADuser/Price"},{title:"광고신청",link:"/Home/ADuser/Subscription"}]},
    {title:"빅데이터",link:"/Home/data",sub:[]},
    {title:"내 광고",link:"/Home/ADuser/MyAD",sub:[]},
...
const [SelectItems, setSelectItems] = useState("홈")
  ...
  }, [dispatch]); // 이곳에서 dispatch만 의존성 배열에 넣을 수 있음, 로그인 처리 자체는 여기서 실행
 ...
 <div onClick={()=>router.push("/")} className=" mx-2 flex flex-col lg:flex-row  items-center ">
            ...
            <div className={`flex-nowrap max-w-[3rem] min-w-[3rem] hidden lg:flex ml-4 justify-center rounded-xl px-1 py-1 text-xs bg-black text-white ${user.role==="admin"?"":"lg:hidden"}`}>{user.role==="admin"?"관리자":""}</div>             
          </div>
          <div className={`flex-nowrap max-w-[3rem] absolute left-[45%] sm:left-[27%] lg:hidden lg:none rounded-xl px-1 py-1 text-xs bg-black text-white ${user.role==="admin"?"":"hidden"}`}>{user.role==="admin"?"관리자":""}</div>           
          <div className=" justify-end lg:justify-between flex lg:text-[1rem] space-x-3 lg:space-x-6 text-xs md:text-base text-dimgray-200 min-w-[10rem] lg:ml-20">
            <div className="hidden lg:flex items-center space-x-2 cursor-pointer">
              <Image
                ...
                className="h-5 w-5"
              />
              <span onClick={()=>user.userId?clickMenu("/Login/Myprofile"):clickMenu("/Login/Siteup")} className="font-extrabold text-nowrap">{user.userId?"내 정보":"회원가입"}</span>
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-2 cursor-pointer">
              <Image
                ...
                className="h-5 w-5"
              />
              <span onClick={()=>user.userId?handleLogout():clickMenu("/Login")} className="font-extrabold text-nowrap">{user.userId?"로그아웃":"로그인"}</span>
            </div>
            <div className="hidden lg:flex items-center space-x-2 cursor-pointer">
              <Image
                src="/rockon.svg"
            ...
            <div  className="lg:hidden justify-end flex items-center cursor-pointer z-50">
              ...
              <Image
                ...
                className="h-5 w-5"
              />
              <span onClick={()=>user.userId?clickMenu("/Login/Myprofile"):clickMenu("/Login/Siteup")} className="font-extrabold">{user.userId?"내 정보":"회원가입"}</span>
            </div>
            <div className="flex lg:hidden items-center space-x-2 cursor-pointer">
              <Image
                ...
                className="h-5 w-5"
              />
              <span onClick={()=>user.userId?handleLogout():clickMenu("/Login")} className="font-extrabold">{user.userId?"로그아웃":"로그인"}</span>
            </div>
               ...
               <div className="w-full h-full flex flex-row justify-between">
                ...
                <div onClick={()=>item.sub.length>0?setSelectItems(()=>item.title):clickMenu(item.link)} className={` transition-all duration-300 px-[2rem]  flex flex-row text-center text-nowrap ${SelectItems===item.title?" bg-gradient-to-r rounded-r-xl scale-110 from-[#0F8EFD] to-[#3DD87A] text-white":""}  py-[1rem] text-md font-bold`}>{item.title}</div>
               </>)):menu.map((item)=>(<>
                <div onClick={()=>item.sub.length>0?setSelectItems(()=>item.title):clickMenu(item.link)} className={` transition-all duration-300 px-[2rem]  flex flex-row text-center text-nowrap ${SelectItems===item.title?" bg-gradient-to-r rounded-r-xl scale-110 from-[#0F8EFD] to-[#3DD87A] text-white":""}  py-[1rem] text-md font-bold`}>{item.title}</div>
               </>))}
</div>
...
{user.role==="admin"?adminmenu.find(item => item.title === SelectItems)?.sub.map((subitem)=>(<>
  <div onClick={()=>clickMenu(subitem.link)} className=" transition-all duration-300 px-[3rem] font-[500]  text-nowrap py-[1rem] w-full">{subitem.title}</div>
 ...
 <div className=" w-full  bg-gainsboro-200 h-[0.063rem]" />
       ...
       {user.role==="admin"?adminmenu.map((item)=>(<>
        ...
        <div className=" cursor-auto h-full top-[0rem] bottom-[0rem] left-[0rem] lg-text  max-w-[16.5rem]">
            <div onClick={()=>item.sub.length>0?"":clickMenu(item.link)} className={`hover:text-dodgerblue top-[calc(50%_-_17px)] left-[calc(50%_-_40px)] leading-[1.25rem] font-medium ${SelectItems===item.title?"text-dodgerblue":""}`}>
              {item.title}
            </div>
          </div>
        </div>
...
{user.role==="admin"?adminmenu.map((item)=>(<>
        ...
        <div className=" h-full top-[0rem] bottom-[0rem] left-[0rem] lg-text flex flex-col max-w-[16.5rem]">
         ...
         {item.sub.map((subitem)=>(<>
          <div onClick={()=>clickMenu(subitem.link)} className={` hover:bg-gradient-to-r hover:rounded-xl hover:scale-110 hover:from-[#0F8EFD] hover:to-[#3DD87A] hover:text-white z-[55]  top-[calc(50%_-_17px)] left-[calc(50%_-_40px)]  leading-[3.25rem] font-medium ${SelectItems===item.title?"text-dodgerblue":""}`}>
              {subitem.title}
            </div>
         </>))}
          </div>
